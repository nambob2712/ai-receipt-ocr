...
from receipt_ocr_system import (
    ReceiptOCRSystem,
    ProcessingStatus,
    ExpenseCategory
)
...
def get_api_key() -> Optional[str]:
    """Get the API key from Streamlit secrets, session state, or environment.
    ...
    # Session state (user input via UI)
...
def check_duplicate_receipt(
    ...
    """
    Check if a receipt with identical content already exists in the database.
    
    A duplicate is defined as a receipt with the same:
    - Date
    - Total amount
    - Merchant name
    - Currency
    ...
    Returns:
        tuple: (is_duplicate: bool, existing_record: Optional[pd.Series])
    """
    ...
    if receipt_date is not None:
        ...
        if 'Date' in df.columns:
            df_dates = df['Date'].dt.strftime('%Y-%m-%d').fillna('')
            conditions &= (df_dates == date_str)
    else:
        # If no date, match with empty dates
        if 'Date' in df.columns:
            conditions &= df['Date'].isna()
    
    # Check total amount match (with small tolerance for floating point)
    if total_amount is not None:
        if 'Total_Amount' in df.columns:
            amount_tolerance = 0.01 if currency != 'JPY' else 0.5
            conditions &= (
                (df['Total_Amount'] >= total_amount - amount_tolerance) &
                (df['Total_Amount'] <= total_amount + amount_tolerance)
            )
    else:
        if 'Total_Amount' in df.columns:
            conditions &= df['Total_Amount'].isna()
    
    # Check merchant match (case-insensitive, trimmed)
    if merchant:
        if 'Merchant' in df.columns:
            merchant_clean = merchant.strip().lower()
            df_merchants = df['Merchant'].fillna('').str.strip().str.lower()
            conditions &= (df_merchants == merchant_clean)
    else:
        if 'Merchant' in df.columns:
            conditions &= (df['Merchant'].fillna('') == '')
    
    # Check currency match
    if 'Currency' in df.columns:
        conditions &= (df['Currency'] == currency)
...
def save_receipt_data(uploaded_file, result) -> bool:
    """
    Save the processed receipt data to persistent storage.
    
    Checks for duplicate receipts before saving. If a duplicate is found,
    displays a warning and prevents saving.
    
    Args:
        uploaded_file: The uploaded file object
        result: The processing result from the OCR system
    ...
    try:
        ...
        is_duplicate, existing_record = check_duplicate_receipt(
        ...
        if is_duplicate:
            ...
            with st.expander("🔍 Duplicate Details", expanded=True):
                st.markdown("This receipt appears to already exist in the database:")
                
                col1, col2 = st.columns(2)
                
                with col1:
                    st.markdown("**Current Receipt:**")
                    st.write(f"• Date: {data.date.strftime('%Y-%m-%d') if data.date else 'N/A'}")
                    st.write(f"• Amount: ¥{data.total_amount:,.0f}" if data.total_amount else "• Amount: N/A")
                    st.write(f"• Merchant: {data.merchant_name or 'N/A'}")
                
                with col2:
                    st.markdown("**Existing Record:**")
                    if existing_record is not None:
                        existing_date = existing_record.get('Date', 'N/A')
                        if pd.notna(existing_date):
                            existing_date = pd.to_datetime(existing_date).strftime('%Y-%m-%d')
                        else:
                            existing_date = 'N/A'
                        
                        existing_amount = existing_record.get('Total_Amount', None)
                        if pd.notna(existing_amount):
                            existing_amount_str = f"¥{float(existing_amount):,.0f}"
                        else:
                            existing_amount_str = 'N/A'
                        
                        existing_merchant = existing_record.get('Merchant', 'N/A')
                        if pd.isna(existing_merchant):
                            existing_merchant = 'N/A'
                        
                        existing_timestamp = existing_record.get('Timestamp', 'N/A')
                        if pd.notna(existing_timestamp):
                            existing_timestamp = pd.to_datetime(existing_timestamp).strftime('%Y-%m-%d %H:%M')
                        
                        st.write(f"• Date: {existing_date}")
                        st.write(f"• Amount: {existing_amount_str}")
                        st.write(f"• Merchant: {existing_merchant}")
                        st.write(f"• Saved on: {existing_timestamp}")
            
            st.error("❌ Receipt was NOT saved to avoid duplication.")
            st.info("💡 If this is a different receipt with the same details, please modify the date or amount slightly, or add a note to distinguish it.")
